...
export default function AdminExportarPage() {
  ...
  const exportarCSV = async (tipo: string) => {
    setExportando(tipo);
    try {
      const path = tipo === "auditoria" ? "/admin/auditoria/export" : `/admin/export/${tipo}`;
  ...
  const opcoes = [
    ...
    { tipo: "auditoria", label: "Auditoria", descricao: "Log de ações do sistema (admin, avaliador, etc.)" },
  ];
