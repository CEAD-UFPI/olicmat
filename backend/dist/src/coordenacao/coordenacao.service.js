...
let CoordenacaoService = class CoordenacaoService {
    prisma;
    constructor(prisma) {
        this.prisma = prisma;
    }
    async getCoordenadorCursos(coordenadorId) {
    ...
    async listAlunos(coordenadorId) {
    ...
    async listInscricoes(coordenadorId, filters) {
        const cursosIds = await this.getCoordenadorCursos(coordenadorId);
        if (cursosIds.length === 0) {
            throw new ForbiddenException("Você não coordena nenhum curso");
        }
    ...
    async getMetricas(coordenadorId) {
        const cursosIds = await this.getCoordenadorCursos(coordenadorId);
        if (cursosIds.length === 0) {
            throw new ForbiddenException("Você não coordena nenhum curso");
        }
...
CoordenacaoService = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [PrismaService])
], CoordenacaoService);
export { CoordenacaoService };
//# sourceMappingURL=coordenacao.service.js.map