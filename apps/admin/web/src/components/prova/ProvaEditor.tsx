...
export function ProvaEditor({
  basePath,
  showErrorBackButton = false,
  idSuffix = "",
}: ProvaEditorProps) {
  const params = useParams();
  const router = useRouter();
  const id = params.id as string;
  const listaPath = `${basePath}/provas`;
  ...
  const salvar = async () => {
    ...
    try {
      ...
      if (id === "nova") {
        ...
        router.push(`${basePath}/provas/${data.id}`);
      } else {
    ...
    return (
      <div className="border border-[#2a2a3a] rounded-2xl p-8 bg-[#12121a] text-center">
        ...
        {showErrorBackButton && (
          <Button
            variant="outline"
            className="mt-4 border-[#2a2a3a] text-[#f0ece4] cursor-pointer"
            onClick={() => router.push(listaPath)}
          >
            Voltar
          </Button>
        )}
    ...
    >
      ...
      <Card className="border-[#2a2a3a] bg-[#12121a]">
        ...
        <CardContent className="space-y-4">
          <div>
            <Label htmlFor={`titulo${idSuffix}`} className="text-[#f0ece4]">Título</Label>
            <Input
              id={`titulo${idSuffix}`}
              value={titulo}
          ...
          <div>
            <Label htmlFor={`duracao${idSuffix}`} className="text-[#f0ece4]">Duração (minutos)</Label>
            <Input
              id={`duracao${idSuffix}`}
              type="number"
              value={duracao}
